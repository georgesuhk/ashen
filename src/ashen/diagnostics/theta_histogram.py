...
``plot_theta_histogram_matrix`` (``Columbia/NL_kinks/prod_plots_draft0.ipynb``,
cell 5): for each traced field line, find the first puncture whose flux
surface exceeds a target ``psi_n`` and record the poloidal angle ``theta`` at
that puncture -- i.e. where the line first crosses out past a chosen surface.
Histogramming those angles across many field lines gives a strike-position /
loss-angle distribution.

No matplotlib import here, matching :mod:`ashen.diagnostics.connection_length`
-- usable from a notebook or a test without pulling in a plotting backend.
...
identical to ``psi_n_ij > target_psi * real_psi_edge``. Since ``target_psi``
is a user-facing *plasma-fraction* quantity and ``record.psi_n`` is already
JOREK-grid (see ``KNOWN_ISSUES.md`` #7 for the settled distinction), scaling
the threshold is the correct-by-construction form of the "apply
``real_psi_edge`` exactly once" rule -- unlike
``connection_length.line_connection_length``, whose legacy double-division
this deliberately does *not* replicate. There is nothing to fix here.
"""
...
class CrossingResult:
    ...
    The notebook silently discarded the crossed/considered counts -- this is
    the fastest way to spot a ``target_psi`` set so high (or a ``psi_n_range``
...
def crossing_angles(
    ...
    *,
    ...
    ``psi_n_range`` (if given) is a ``[min, max]`` filter on each line's
    *starting* ``psi_n`` (``LineKey.psi_n``) -- user-facing plasma-fraction
    bounds, scaled by ``real_psi_edge`` the same way ``target_psi`` is, since
    ``LineKey.psi_n`` is also JOREK-grid. Replaces the notebook's ``i_lim``
    (a positional index into scan order, which silently changes meaning
    whenever ``psi_n_in`` is widened or reordered -- the exact hazard the
    per-line cache was keyed by starting position to remove).

    A line whose punctures never cross the threshold (confined) contributes
    no angle but is still counted in ``n_considered``. A line's own starting
    ``psi_n`` can never already exceed the threshold: ``jorek2_postproc``'s
    ``fluxsurface`` command rejects any request outside ``[0, 1]``
    (``exec_commands.f90:3063-3068``), so every start is ``<= real_psi_edge``
    while the threshold is ``target_psi * real_psi_edge`` with
    ``target_psi > 1`` -- there is no "already past threshold at the start"
...
def theta_histogram(angles: np.ndarray, *, bins: int) -> tuple[np.ndarray, np.ndarray]:
    """Bin ``angles`` over ``(-pi, pi)``, weighted to the fraction of lines
    per bin -- ports the notebook's ``weights = np.ones_like(data) /
    len(data)`` exactly (George's call: keep the legacy fraction
    normalisation; the y-limit is auto-scaled at draw time instead of the
    notebook's hardcoded 0.06).

    Returns ``(counts, bin_edges)``, matching ``np.histogram``'s convention.
    An empty ``angles`` returns all-zero counts over the full range rather
    than raising, so a panel with no crossings still has bin edges to share
    an x-axis with its neighbours.
    """
    if angles.size == 0:
